...
import { getProviderFamily } from '../data/providers';
...
export function generateFiles(context: InitContext): FileToWrite[] {
  ...
  files.push(...generateCustomProviderFiles(context));
...
function findProviderModel(familyId: string, modelId: string): ProviderModel | undefined {
  return getProviderFamily(familyId)?.models.find((model) => model.id === modelId);
}

function generateCustomProviderFiles(context: InitContext): FileToWrite[] {
  const files: FileToWrite[] = [];
  const providerIds = context.providers.flatMap((provider) => provider.models);

  if (providerIds.includes('file://provider.py')) {
    files.push({
      path: `${context.outputDirectory}/provider.py`,
      relativePath: 'provider.py',
      content: generatePythonProviderStub(),
      exists: false,
      overwrite: true,
    });
  }

  if (providerIds.includes('file://provider.js')) {
    files.push({
      path: `${context.outputDirectory}/provider.js`,
      relativePath: 'provider.js',
      content: generateJavaScriptProviderStub(),
      exists: false,
      overwrite: true,
    });
  }
...
function generateProviderConfig(
  ...
  return providers.flatMap((provider) =>
    provider.models.map((modelId) => {
      const model = findProviderModel(provider.family, modelId);
      const config = {
        ...(model?.config || {}),
        ...(provider.config || {}),
      };

      return Object.keys(config).length > 0 ? { id: modelId, config } : modelId;
    }),
  );
}
...
function generatePythonProviderStub(): string {
  return `# Learn more: https://promptfoo.dev/docs/providers/python/

def call_api(prompt, options, context):
    return {
        "output": f"Stub response for prompt: {prompt}",
    }
`;
}

function generateJavaScriptProviderStub(): string {
  return `// Learn more: https://promptfoo.dev/docs/providers/custom-api
...
class CustomApiProvider {
  ...
  async callApi(prompt) {
    return {
      output: \`Stub response for prompt: \${prompt}\`,
    };
  }
}
